...
const buildPayload = (input) => {
  ...
  return {
    ...input,
    // forward sampleConfig/seed unchanged so backend can prefer local samples
    sampleConfig: input.sampleConfig || {},
    seed: input.seed || `local-${Date.now()}`,
...
const buildLocalPattern = (params) => {
  ...
  Object.keys(pattern).forEach((inst) => {
    sampleMapping[inst] = (params.sampleConfig && params.sampleConfig[inst]) || params.sampleConfig?.default || `builtin:${inst}`;
  });
...
export const AIWorkflow = {
  async generateDrums(params) {
    ...
    // ensure sampleMapping present
    if (!data.sampleMapping) {
      data.sampleMapping = {};
      const provided = params.sampleConfig || {};
      Object.keys(data.pattern || {}).forEach((inst) => {
        data.sampleMapping[inst] = provided[inst] || provided.default || `builtin:${inst}`;
      });
    }
